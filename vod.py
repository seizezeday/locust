...
GET_FREQUENCY_BASE = 1
VOD_FREQUENCY_BASE = 1
ARM_FREQUENCY_BASE = 1
RELOGIN_FREQUENCY = 1
...
class VisonicRestApiLoadScenario(TaskSet):
    ...
    def is_panel_connected(self):
        response = self.client.get("/rest_api/2.0/status", headers = {'Session-Token':'%s'%self.client.token},
                                   name = "get_status", verify = False)
    ...
    def make_vod(self):
        while(True):
            ...
            if (self.is_panel_connected()):
                time.sleep(5)
                camera_id = random.choice([3,23])
                self.make_video(camera_id)
                break
            else:
